...
test('brand mascot lives at the corner; sidebar head is title plus collapse', () => {
  ...
  const composer = readFileSync(resolve(import.meta.dirname, '../../../cap-chat/src/web/composer.tsx'), 'utf8')
  assert.match(composer, /BrandCornerMascot/)
  assert.match(composer, /variant="popover"/)
  ...
  assert.match(css, /\.composer-caret-mascot \.brand-corner-cluster\s*\{[^}]*position:\s*relative/s)
...
test('shell columns stay three tracks without animating on window resize', () => {
  ...
  assert.doesNotMatch(css, /\.sidebar-flyout-host\.is-collapsed:hover[^{]*\{[^}]*width:\s*var\(--sidebar-flyout-width/)
