...
test('sessionsCollection maps summaries and writes title/pinned/tags', async () => {
  ...
  assert.deepEqual(spec.actions?.find((item) => item.id === 'progress')?.for, 'agent')
  assert.deepEqual(spec.actions?.find((item) => item.id === 'compact')?.for, 'agent')
  assert.deepEqual(spec.actions?.find((item) => item.id === 'inspect')?.for, 'agent')
})
