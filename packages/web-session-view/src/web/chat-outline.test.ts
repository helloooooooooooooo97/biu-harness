...
test('message outline floats beside a centered trigger', () => {
  ...
  assert.match(css, /\.chat-outline-icon-btn\s*\{[^}]*background:\s*transparent/s)
  assert.match(css, /\.chat-outline-icon-btn\s*\{[^}]*width:\s*40px/s)
  assert.doesNotMatch(css, /\.chat-outline-icon-btn\s*\{[^}]*border-radius:\s*999px/s)
