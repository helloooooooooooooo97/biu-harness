...
test('create record sits at the right of the toolbar with a blue label', () => {
  ...
  assert.match(css, /\.fsdb-page \.tasks-table-wrap\{[^}]*padding-left:26px/)
  assert.match(css, /\.fsdb-page \.tasks-table \.fsdb-row-check\{[^}]*left:-24px/)
  assert.match(css, /tbody tr:hover \.fsdb-row-check/)
  assert.match(css, /\.fsdb-page \.tasks-table thead \.fsdb-row-check\{[^}]*opacity:1/)
  assert.match(browser, /index === 0 \? <RowCheck ids=\{pickableIds\} \/> : null/)
  assert.match(css, /\.tasks-minicard-title\{[^}]*font-size:22px/)
...
test('database extras sit after the record detail, not in the inspector', () => {
  ...
  assert.match(style, /\.fsdb-detail-main\{[^}]*padding:80px 24px 24px/)
  assert.match(style, /\.fsdb-main > \.fsdb-detail-title-row\{[^}]*flex:none/)
  assert.match(style, /\.fsdb-main\{[^}]*padding:80px 24px 16px/)
