...
test('create record sits at the right of the toolbar with a blue label', () => {
  ...
  assert.match(css, /\.fsdb-row-check\{[^}]*right:100%/)
...
test('database extras sit after the record detail, not in the inspector', () => {
  ...
  assert.match(style, /\.fsdb-detail-float-nav\{[^}]*background:rgba\(255,255,255,\.10\)/)
  assert.match(style, /\.fsdb-detail-float-nav\{[^}]*backdrop-filter:blur\(4px\)/)
  assert.match(style, /\.fsdb-detail-float-btn\{[^}]*width:24px/)
  assert.match(style, /\.fsdb-detail-float-btn svg\{[^}]*width:14px/)
  assert.match(style, /\.fsdb-detail-float-btn\{[^}]*color:#a3a3a3/)
