...
test('table title opens record from the title-side button', () => {
  ...
  assert.match(browser, /onOpenRow\?\.\(row\)/)
  ...
  const page = readFileSync(resolve(import.meta.dirname, './index.tsx'), 'utf8')
  assert.match(page, /openRegisteredRow/)
  assert.match(page, /onOpenRow=\{/)
  assert.match(page, /addSchemaTagField/)
  const nav = readFileSync(resolve(import.meta.dirname, './collection-nav.ts'), 'utf8')
  assert.match(nav, /catalogRowOpenTarget/)
  assert.match(nav, /tagRowOpenTarget/)
  assert.match(nav, /stampRowOpenTarget/)
  assert.match(browser, /onAddField/)
  assert.match(browser, /<AddProperty/)
