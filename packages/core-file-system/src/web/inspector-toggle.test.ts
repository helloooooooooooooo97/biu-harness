...
test('create record sits at the right of the toolbar with a blue label', () => {
  ...
  assert.match(css, /\.fsdb-page \.tasks-row-tools-slot\{[^}]*width:var\(--fsdb-row-tools-w\)/)
  ...
  assert.match(browser, /<SchemaFieldEditor[\s\S]*compact/)
...
test('card and queue rows expose split and expand like the table', () => {
  ...
  const titleFn = browser.slice(browser.indexOf('function RecordTitle'), browser.indexOf('function TableRecordIcon'))
  const cardFn = browser.slice(browser.indexOf('function MiniCard'), browser.indexOf('function TableBodyRows'))
  const queueFn = browser.slice(browser.indexOf('function QueueRow'), browser.indexOf('function MiniCard'))
  assert.match(titleFn, /<RecordOpenControls row=\{row\}/)
  assert.match(titleFn, /<RecordRowTools row=\{row\}/)
  assert.doesNotMatch(cardFn, /RecordRowTools/)
  assert.doesNotMatch(cardFn, /RecordOpenControls/)
  assert.doesNotMatch(cardFn, /tasks-minicard-bar/)
  assert.doesNotMatch(queueFn, /RecordRowTools/)
  assert.doesNotMatch(queueFn, /RecordOpenControls/)
  assert.doesNotMatch(queueFn, /tasks-queue-item-tools/)
  ...
  assert.match(style, /\.fsdb-page \.tasks-minicard \.tasks-row-tools[\s\S]*opacity:0/)
  assert.match(style, /\.fsdb-page \.tasks-queue-item \.tasks-row-tools[\s\S]*opacity:0/)
})
...
test('database extras sit after the record detail, not in the inspector', () => {
  ...
  assert.match(outline, /SidebarOutlinePortal/)
  assert.match(outline, /heading-outline-host/)
