...
test('list and detail share the chat column max width with side padding', () => {
  ...
  assert.match(css, /\.fsdb-right-body:has\(\.chat-pane-embed\)\{[^}]*overflow:hidden/)
  assert.match(css, /\.fsdb-fileview:has\(\.chat-pane-embed\)\{[^}]*height:0/)
  assert.match(css, /\.fsdb-detail-main:has\(\.chat-pane-embed\)\{[^}]*padding:8px 12px 0/)
  assert.match(css, /\.fsdb-detail-main:has\(\.chat-pane-embed\) \.fsdb-detail-aside\{display:none\}/)
