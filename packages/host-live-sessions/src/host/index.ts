...
工作流：用 db_list /sessions 和 db_action action=inspect / progress 了解现场；新建用 db_create /sessions，改元数据用 db_update 或 db_action configure/tag/star。派工必须走任务体系：db_create /tasks、db_update 指派 assigneeSessionId、db_action action=deliver 派发、action=report 上报；不要直接 dispatch。废弃会话用 db_delete /sessions/<id>。
