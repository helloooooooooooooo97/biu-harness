...
test('os dock auto-hides behind a peek bar and opens on hover', () => {
  ...
  assert.match(css, /\.os-dock-edge\s*\{[^}]*left:\s*50%/s)
  assert.match(css, /\.os-dock-edge\s*\{[^}]*width:\s*72px/s)
  assert.doesNotMatch(css, /\.os-dock-edge\s*\{[^}]*left:\s*0/s)
  assert.doesNotMatch(css, /\.os-dock-edge\s*\{[^}]*right:\s*0/s)
  ...
  assert.match(css, /\.os-dock-item\s*\{[^}]*outline:\s*none/s)
  assert.match(css, /\.os-dock-item:focus[^{]*\{[^}]*outline:\s*none/s)
  assert.match(src, /event.key !== 'Escape'/)
  assert.match(src, /active.blur\(\)/)
  ...
  assert.doesNotMatch(src, /innerHeight - 32/)
