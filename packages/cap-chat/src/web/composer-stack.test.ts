...
describe('composer dock stacking above sticky user', () => {
  ...
  it('uses SuperTag tones for pick chips and the clear-context control, not the plus', () => {
    ...
    expect(css).toMatch(/\.project-chip\.project-chip-pick-toggle\.is-active\s*\{[^}]*#5b9fd6/s)
  ...
  it('uses SuperTag rose fill on clear-context only when there is history', () => {
    ...
    expect(css).toMatch(/\.tool-call-status\.is-ok\s*\{[^}]*color-mix\(in srgb, #448361 22%, transparent\)/s)
    expect(css).toMatch(/\.tool-call-status\.is-fail\s*\{[^}]*color:\s*#c4554d/s)
    expect(css).toMatch(/\.tool-call-status\.is-fail\s*\{[^}]*color-mix\(in srgb, #c4554d 22%, transparent\)/s)
    expect(css).toMatch(/\.traj-usage-ring\.is-cache\s*\{[^}]*color:\s*#448361/s)
    expect(css).toMatch(/\.traj-usage-ring\.is-hist\s*\{[^}]*color:\s*#c4554d/s)
    const usage = readFileSync(resolve(root, 'packages/cap-chat/src/web/usage-inline.tsx'), 'utf8')
    expect(usage).toMatch(/TAG_TONE_GREEN/)
    expect(usage).toMatch(/TAG_TONE_RED/)
