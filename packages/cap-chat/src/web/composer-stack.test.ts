...
describe('composer dock stacking above sticky user', () => {
  ...
  it('puts the session mascot after the header name, with the dock session picker', () => {
    ...
    expect(css).toMatch(/\.chat-overlay-panel \{/)
    expect(css).toMatch(/\.chat-overlay-panel\s*\{[^}]*backdrop-filter:\s*blur\(16px\)\s*saturate\(1\.2\)/s)
    expect(css).toMatch(/\.chat-overlay-panel\s*\{[^}]*background:\s*rgba\(255,\s*255,\s*255,\s*0\.1\)/s)
