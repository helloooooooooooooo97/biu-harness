...
describe('composer dock stacking above sticky user', () => {
  it('isolates chat-stage and keeps composer-dock above sticky user z-index', () => {
    ...
    expect(css).toMatch(/\.chat-composer-dock\.is-scroll-faded\s*\{[^}]*opacity:\s*0/s)
    ...
    expect(shell).toContain('px-[60px]')
  ...
  it('puts the session mascot after the header name, with the dock session picker', () => {
    ...
    expect(css).toMatch(/\.chat-pane-embed\s*\{[^}]*overflow:\s*hidden/s)
    expect(css).toMatch(/\.chat-pane-embed \.chat-overlay-thread\s*\{[^}]*overflow:\s*hidden/s)
