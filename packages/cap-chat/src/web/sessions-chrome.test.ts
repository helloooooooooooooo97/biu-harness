...
test('session table uses mascot as the standalone icon property, title is just the label', () => {
  ...
  assert.match(src, /\.fsdb-detail-main:has\(\.chat-pane-embed\) \.chat-stage\{[^}]*align-items:stretch/)
  assert.match(src, /\.fsdb-detail-main:has\(\.chat-pane-embed\) \.chat-stage>\*\{[^}]*max-width:none/)
  assert.match(src, /\.fsdb-detail-main:has\(\.chat-pane-embed\) \.chat-pane-embed\{[^}]*padding:0/)
