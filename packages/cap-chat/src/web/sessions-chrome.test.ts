...
test('session table uses mascot as the standalone icon property, title is just the label', () => {
  ...
  assert.match(src, /\.fsdb-detail-stage:has\(\.chat-pane-embed\)/)
  assert.match(src, /\.fsdb-fileview:has\(\.chat-pane-embed\)\{[^}]*min-height:0/)
  assert.doesNotMatch(src, /min-height:min\(72vh,720px\)/)
