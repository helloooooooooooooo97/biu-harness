...
test('session table uses mascot as the standalone icon property, title is just the label', () => {
  ...
  assert.doesNotMatch(src, /fsdb-detail-stage:has\(\.chat-pane-embed\),/)
  assert.match(src, /fsdb-detail-stage:has\(\.chat-pane-embed\) \.fsdb-detail-float-nav/)
  ...
  assert.match(src, /session-composer-host composer-dock-zone/)
