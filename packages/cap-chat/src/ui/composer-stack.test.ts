...
describe('composer dock stacking above sticky user', () => {
  it('isolates chat-stage and keeps composer-dock above sticky user z-index', () => {
    ...
    const shell = readFileSync(resolve(root, 'packages/web-app-shell/src/index.tsx'), 'utf8')
    const thread = readFileSync(resolve(root, 'packages/cap-chat/src/ui/thread.tsx'), 'utf8')
