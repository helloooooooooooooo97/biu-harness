...
主仓 **`host/` / `src/` 只留加载器**：读 `cordis.plugins.json`，按包名动态 import。插件源码在 `packages/`，包名一律 `@biu/<prefix>-<id>`。
...
| `web` | `src/main.tsx` ← `virtual:cordis-web-runtime` | 壳（`web-slots` / `web-app-shell` / `web-ui-hub` …） |
| `plugins` | `@biu/host-hub` + `@biu/web-ui-hub` | 可热插拔能力；`ui` 字段成对卸前端 |
...
- **`host-*`**：host 内核插件（无浏览器入口）。
- **`web-*`**：web 内核插件；`web-mascot` 是共享库，不必进 json。
- **`cap-*`**：能力插件，同一目录，**`exports` 必须把 `./host` 与 `./ui` 分开**（禁止一个入口同时带 Node + React）。json 写 `"package": "@biu/cap-chat/host", "ui": "@biu/cap-chat/ui"`。
