...
`host/index.ts` 只做：建 Context、读 `host` 表、`plugin()`。`hub` 也是 `@hmr/host-runtime/hub`，写在 `host` 表最后一项（它再去挂 `plugins` 表）。
...
| `plugins` | `@hmr/host-runtime/hub` + web `ui-hub` | 可热插拔能力；`ui` 字段成对卸前端 |
