...
/** 按 provider 名注册/取用适配器。 */
export class AdapterRegistry {
  private readonly adapters = new Map<string, LlmAdapter>()

  register(adapter: LlmAdapter): () => void {
    if (this.adapters.has(adapter.provider)) throw new Error(`适配器已存在: ${adapter.provider}`)
  ...
  get(provider: string): LlmAdapter {
    ...
    if (!adapter) throw new Error(`未知适配器: ${provider}`)
    return adapter
  }
...
/** 把流式 chunk 组装成一条 assistant 消息（text/reasoning 合并、tool-call 增量拼接）。 */
export function assemble(chunks: AsyncIterable<StreamChunk>): Promise<Message> {
  return (async () => {
