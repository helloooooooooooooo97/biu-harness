...
2. **生成骨架**：运行 `bash code/scaffold.sh --dir /tmp/mini-dsh-demo`，然后用 `find /tmp/mini-dsh-demo -maxdepth 2 -type d` 检查结构；再跑一次 `--dry-run` 对比。
3. **空目录保护**：故意在一个已有文件的目录上运行 scaffold（如 `bash code/scaffold.sh --dir .`），确认脚本拒绝覆盖。
