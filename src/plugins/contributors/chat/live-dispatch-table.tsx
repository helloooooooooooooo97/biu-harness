      ...
      <table className="w-full border-collapse text-left text-[11px]">
        ...
        <tbody>
          {tasks.map((task, index) => (
            ...
            >
              ...
              <td className={`whitespace-nowrap px-2 py-2 align-top font-semibold ${statusClass(task.status)}`}>
                {statusLabel(task.status, task.reason)}
              </td>
              <td className="whitespace-nowrap px-3 py-2 align-top text-right tabular-nums text-[var(--dsw-label-2)]">
                {task.usage
        ...
        <tfoot>
          <tr className="border-t border-[var(--dsw-border)] bg-[color-mix(in_srgb,var(--dsw-hover)_50%,transparent)]">
            <td className="px-3 py-2 font-semibold text-[var(--dsw-label)]" colSpan={2}>
              合计（{tasks.length}）
            </td>
            <td
              className="px-3 py-2 text-right font-semibold tabular-nums text-[var(--dsw-label)]"
